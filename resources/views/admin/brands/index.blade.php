...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <div class="row m-20">
      <div class="col-sm-3">
        <input type="text" id="search" placeholder="Search Brands..." value="{{ $search }}" class="form-control">
      </div>
      ...
      <div class="col-md-10">
        <div class="card card-primary">
              ...
              <tbody>
                @if(count($brands) > 0)
                  ...
                  <tr>
                    ...
                    <td align="center">
                      @if($value->image != '')
                        <img class="img-thumbnail" src="{{url($value->image)}}" style="max-height:50px; max-width: 50px;">
                      @else 
                        <img class="img-thumbnail" src="{{url('uploads/image-not-found.png')}}" style="max-height:45px; max-width: 45px;">
                      @endif
                    </td>
                    <td>{{$value->name}}</td>
                    <td align="center">{{$value->disp_order}}</td>
                    <?php $count = App\Http\Controllers\AdminBrandsController::countProducts($value->id); ?>
                    <td align="center">{{$count}}</td>
                    <td align="center">
                      <div class="row">
                        <div class="col-sm-6" align="right">
                          <a href="" class="btn btn-sm btn-warning" title="Edit Brand" data-toggle="modal" data-target="#modal-add" onclick='mkeEditForm("{{$value->id}}","{{$value->name}}", "{{$value->disp_order}}", "{{$value->image}}")' style="color: white;">
                            <i class="fa fa-edit" style="font-size:16px"></i> <b>Edit</b>
                          </a>
                        </div>
                        <div class="col-sm-6" align="left">
                          <form action="{{url('/admin/brands/delete/'.$value->id)}}" method="POST" role="form" id="delform{{$value->id}}">
                            {!! csrf_field() !!}
                            <button type="button" class="btn btn-sm btn-danger" title="Delete Brand" data-toggle="modal" data-target="#modal-delete" onclick='mkeDelModal("{{$value->id}}","{{$value->name}}");' @if($count > 0) disabled @endif >
          ...
          <div class="card-footer clearfix ">
            @if(count($brands) > 0)
              {{$brands->appends( array("search" => $search))->links()}}
            @endif
          </div>
        </div>
        <!-- /.card -->
    ...
    <!-- Add new Banner link -->
    <a href="" data-toggle="modal" data-target="#modal-add" title="Add New Brand" onclick="mkeAddForm();">
  ...
  <div class="modal fade" id="modal-add">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <form action="{{url('/admin/brands/create')}}" method="POST" id="addform" role="form" enctype="multipart/form-data">
          {!! csrf_field() !!}
          <input type="hidden" name="id" id="id" value="">

          <div class="modal-header">
            <h4 class="modal-title" id="bannerHeading">Add New Brand</h4>
          ...
          <div class="modal-body">
            ...
            <div class="card-body">
              <div class="form-group row">
                ...
                <div class="col-sm-7">
                  <input class="form-control" placeholder="Enter Brand Name" name="name" type="text" value="" id="name">
                  <span class="invalid-feedback">Please enter Brand Name.</span>
                </div>
              </div>
              ...
              <div class="form-group row">
                <label for="image" class="col-sm-3 col-form-label text-right">Brand Icon:</label>
                <div class="col-sm-7">
  ...
  <div class="modal fade" id="modal-delete">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <h5>Are You Sure ?</h5>
          <p>If you Delete this Brand, All Products Associated with it may be Effected.</p>
        </div>
        <div class="modal-footer justify-content-between">
          <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
  ...
  <input type="hidden" id="adddisp_order" value="<?php echo $disp_order ?? '1'; ?>">
...
<script>
  ...
  $('#searchBtn').click(function() {
    var url = '{{url('/admin/brands')}}?search=' + $('#search').val();
  ...
  function mkeAddForm() {
    $('#addform').attr('action', '{{url('/admin/brands/create')}}');
    $('#name,#id').val('');
    $('#bannerHeading').html('Add New Brand');
    $('#name').removeClass('is-invalid');
    $('#disp_order').val($('#adddisp_order').val());
    $('#helpImage').html('').addClass('hide');
    $('#image').removeClass('is-invalid');
    $('#showImage').attr('src', '');
    $('#showImage').addClass('hide');
  ...
  function mkeEditForm(id, name, disp_order, image) {
    $('#name').removeClass('is-invalid');
    $('#addform').attr('action', '{{url('/admin/brands/update')}}');
    $('#bannerHeading').html('Update Brand');
    $('#id').val(id);
    $('#name').val(name);
